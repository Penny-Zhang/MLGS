...
class SmoothClassifier(nn.Module):
    ...
    def certify(self, inputs: torch.Tensor, n0: int, num_samples: int, 
        ...
        top_class = torch.argmax(class_counts).item()
        class_cert = self._sample_noise_predictions(inputs, num_samples, batch_size)
        n_A = class_cert[top_class].item()
        p_A_lower_bound = lower_confidence_bound(n_A, num_samples, alpha)
        ##########################################################
    ...
    def predict(self, inputs: torch.tensor, num_samples: int, alpha: float, 
        ...
        ##########################################################
        # YOUR CODE HERE
        top_1 = max(class_counts)
        top_class = torch.argmax(class_counts).item()
        class_counts_copy = class_counts.clone()
        class_counts_copy[top_class] = 0
        top_2 = max(class_counts_copy)
        
        if binom_test(top_1, top_1 + top_2, p = 0.5) <= alpha:
            return top_class
        else:
            return -1
        ##########################################################
    ...
    def _sample_noise_predictions(self, inputs: torch.tensor, num_samples: int, 
        ...
        with torch.no_grad():
            ...
            for it in range(ceil(num_samples / batch_size)):
                ...
                this_batch = inputs.repeat(this_batch_size, 1, 1, 1) # 1- N
                logits = self.forward(this_batch)
                top_classes = torch.argmax(logits, dim = 1)
